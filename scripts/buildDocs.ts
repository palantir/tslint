...
import { isLatest } from "../lib/configs/latest";
import { isRecommended } from "../lib/configs/recommended";
...
function buildSingleModuleDocumentation(
    ...
    if (DocumentedItem !== null && DocumentedItem.metadata !== null) {
        ...
        // Add additional properties only if a rule
        if (isRuleMetadata(metadata)) {
            metadata.isRecommended = isRecommended(moduleName);
            metadata.isLatest = isLatest(moduleName);
        }
...
function isRuleMetadata(metadata: Metadata): metadata is IRuleMetadata {
    return !!(metadata as IRuleMetadata).ruleName;
}
