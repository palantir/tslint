...
export class WalkContext<T> {
    ...
    constructor(public readonly sourceFile: ts.SourceFile, public readonly ruleSeverity: RuleSeverity,
                public readonly ruleName: string, public readonly options: T) {}
    ...
    public addFailure(start: number, end: number, failure: string, fix?: Fix) {
        ...
        this.failures.push(
            new RuleFailure(this.sourceFile, Math.min(start, fileLength), Math.min(end, fileLength), failure,
              this.ruleSeverity, this.ruleName, fix),
        );
    }
