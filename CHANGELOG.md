...
- **BREAKING CHANGES**
  ...
  - These changes affect custom rules:
    - [api] Remove use of the Language Service. Use only the Program. (#2235)
    - [api] Remove `createFix`. Replacements should be passed directly into `addFailure`. (#2403)
...
- :fire: **New: Configurable Rule Severity**
  ...
  - [config] Adds `defaultSeverity` with options `error` (default), `warning`, and `off`. (#2416)
    ...
    - New style (`callable-types` outputs errors and `max-line-length` outputs warnings):
...
- [new-rule-option] `prefer-const` `{destructuring: "all"}` to only warn if all variables in a destructuring can be const (#2219)
- [new-rule-option] `no-trailing-whitespace` added `ignore-template-strings` (#2359)
- [new-rule-option] `variable-name` adds "allow-snake-case" option
